...
class SigninForm(FlaskForm):
    """Login form with optional "Remember Me" functionality."""
    username = StringField("Username", validators=[DataRequired()])
    password = PasswordField("Password", validators=[DataRequired()])
...
class ImportTimetableForm(FlaskForm):
    ics_file = FileField('Upload Your ICS Timetable File', 
                        validators=[Optional(), FileAllowed(['ics'], "Only Support .ics file!")])
    ics_url = StringField('Your Personalised ICS file URL', 
                         validators=[Optional(), URL(message="Please input valid URL")])
